...
const APP_URL =
  (process.env.NEXT_PUBLIC_APP_URL || 'https://fastrest.app').replace(/\/$/, '')

function buildTableUrl(tableId: string): string {
  return `${APP_URL}/menu?table=${tableId}`
}
...
export default function TablesPage() {
  ...
  const handleAdd = async () => {
    ...
    const exists = tables.some(
      (t) =>
        t.number === num &&
        t.room.toLowerCase() === room.trim().toLowerCase()
    )
    ...
    try {
      ...
      const qrUrl = buildTableUrl(ref.id)
      ...
      console.error('QR yaratish xatosi:', e)
      setError('QR yaratishda xato yuz berdi. Qayta urining.')
    } finally {
      setAdding(false)
    }
  }
    ...
    <div className="min-h-screen p-4" style={{ background: '#0B0B0F' }}>
      ...
      <div className="mb-6 pt-2">
        ...
        <p className="text-gray-400 text-sm mt-1">
          {tables.length} ta stol • QR kod avtomatik yaratiladi
        </p>
      </div>
      ...
      >
        ...
        <div className="flex gap-3 mb-3">
          ...
          <input
            type="text"
            placeholder="Xona nomi (VIP, Asosiy, Terasa...)"
            value={room}
        ...
        >
          {adding ? '⏳ QR yaratilmoqda...' : "✅ Stol qo'shish va QR yaratish"}
        </button>
      </div>
      ...
      <div className="space-y-3">
        {tables.length === 0 && (
          <p className="text-gray-500 text-center py-16">
            Hech qanday stol yo'q. Yuqoridan qo'shing.
          </p>
        )}
        ...
        {tables.map((table) => (
          ...
          >
            {table.qrImage ? (
              ...
              <div
                className="rounded-xl flex items-center justify-center text-xs text-gray-500 shrink-0"
            ...
            <div className="flex-1 min-w-0">
              <p className="text-white font-bold">
                {table.room} — Stol #{table.number}
              </p>
              <p className="text-xs mt-0.5 truncate" style={{ color: '#D4AF37' }}>
                {table.qrUrl || 'Yaratilmoqda...'}
              </p>
              <span
            ...
            <div className="flex flex-col gap-2 shrink-0">
              <button
                onClick={() => table.qrImage && setPreview(table)}
                disabled={!table.qrImage}
                className="px-3 py-1.5 rounded-lg text-xs font-bold"
                style={{
                  background: table.qrImage ? '#D4AF37' : '#333',
                  color: table.qrImage ? '#000' : '#666',
                }}
              >
                👁 Ko'rish
              </button>
              <button
                onClick={() =>
                  table.qrImage &&
                  downloadPng(table.qrImage, `stol-${table.room}-${table.number}.png`)
                }
                disabled={!table.qrImage}
                className="px-3 py-1.5 rounded-lg text-xs font-bold"
                style={{
                  background: 'rgba(0,200,150,0.12)',
                  color: table.qrImage ? '#00C896' : '#444',
                }}
              >
                ⬇ Yuklab
              </button>
      ...
      {preview && (
        ...
        >
          <div
            className="p-6 rounded-3xl max-w-sm w-full text-center"
          ...
          >
            ...
            <p className="text-gray-400 text-xs mb-5 break-all">
              {preview.qrUrl}
            </p>
            <img
              src={preview.qrImage}
            ...
            <div className="flex gap-3">
              <button
                onClick={() =>
                  downloadPng(
                    preview.qrImage,
                    `stol-${preview.room}-${preview.number}.png`
                  )
                }
                className="flex-1 p-3 rounded-xl font-bold text-black"
