...
function MenuContent() {
  ...
  useEffect(() => {
    ...
    // localStorage dan bg o'qi (tezkor)
  ...
  const categories = useMemo(() =>
    ['Hammasi', ...Array.from(new Set(menu.map(m => m.category)))],
    [menu]
  )
  ...
  return (
    <div className="min-h-screen pb-24" style={{
      ...
      <style>{`
        ...
        .menu-card:nth-child(n+7){animation-delay:0.1s}
        .add-btn{transition:transform 0.12s ease,box-shadow 0.15s}
        .add-btn:active{transform:scale(0.85)}
        .add-btn:hover{box-shadow:0 2px 12px rgba(212,175,55,0.5)}
        ...
        .cart-btn:hover{box-shadow:0 4px 20px rgba(212,175,55,0.4)}
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar">
          {categories.map(cat => (
            <button key={cat} onClick={() => setCategory(cat)}
              className="cat-btn px-3 py-1.5 rounded-full text-sm whitespace-nowrap font-medium"
              style={{
